...
int main() {
   ...
   assert(alloc(13) < ptr[1]); // finds ptr[0] when immed coalescing is implemented
   assert(alloc(1) < ptr[4]);  // finds ptr[3] when immed coalescing is implemented

   assert((ptr[5] = alloc(4)) < ptr[4]);  // when immed coalescing is implemented
   ...
   assert((ptr[8] = alloc(400)) == ptr[6]); // when immed coalescing is implmemented
